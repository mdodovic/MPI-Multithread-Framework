...
void compute_tran_temp(float *result, int num_iterations, float *temp, float *power, int row, int col, int rank, int size) {
    ...
    for (int i = 0; i < num_iterations; i++) {
        ...
        if (rank == MASTER) {
            int number_of_jobs = (int) ceil(row / ROWS_PER_JOB);
            int sent_jobs = 0;
            int finished_jobs = 0;
            int not_working = 0;
            int assinged_row = 0;

            while (sent_jobs + not_working < size - 1) {

                if (sent_jobs < number_of_jobs) {
                    sent_jobs++;
                
                    MPI_Send(&assinged_row, 1, MPI_INT, sent_jobs, START_TAG, MPI_COMM_WORLD);
                    
                    if (assinged_row != 0){
                        MPI_Send(t + (assinged_row - 1) * col, 1, single_row, sent_jobs, ROW_TAG, MPI_COMM_WORLD);               
                    }

                    MPI_Send(t + assinged_row * col, ROWS_PER_JOB, single_row, sent_jobs, ROW_TAG, MPI_COMM_WORLD);               
                
                    if ((assinged_row + ROWS_PER_JOB) != row) {
                        MPI_Send(t + (assinged_row + ROWS_PER_JOB) * col, 1, single_row, sent_jobs, ROW_TAG, MPI_COMM_WORLD);               
                    }

                    assinged_row += ROWS_PER_JOB;
                }
                else {
                    not_working++;
                    MPI_Send(&sent_jobs, 1, MPI_INT, sent_jobs + not_working, END_TAG, MPI_COMM_WORLD);
                }
            }

            while (finished_jobs < number_of_jobs) {
                int row_result;
                
                MPI_Recv(&row_result, 1, MPI_INT, MPI_ANY_SOURCE, RESULT_ROW_TAG, MPI_COMM_WORLD, &status);
                
                int source_rank = status.MPI_SOURCE;

                MPI_Recv(r + row_result * col, ROWS_PER_JOB, single_row, source_rank, RESULT_TAG, MPI_COMM_WORLD, &status);
                ...
                if (sent_jobs < number_of_jobs) {
                    ...
                    MPI_Send(&assinged_row, 1, MPI_INT, source_rank, START_TAG, MPI_COMM_WORLD);
                    
                    if (assinged_row != 0){
                        MPI_Send(t + (assinged_row - 1) * col, 1, single_row, source_rank, ROW_TAG, MPI_COMM_WORLD);               
                    }

                    MPI_Send(t + assinged_row * col, ROWS_PER_JOB, single_row, source_rank, ROW_TAG, MPI_COMM_WORLD);               
                
                    if ((assinged_row + ROWS_PER_JOB) != row) {
                        MPI_Send(t + (assinged_row + ROWS_PER_JOB) * col, 1, single_row, source_rank, ROW_TAG, MPI_COMM_WORLD);               
                    }

                    assinged_row += ROWS_PER_JOB;
                }
                else {
                    MPI_Send(&sent_jobs, 1, MPI_INT, source_rank, END_TAG, MPI_COMM_WORLD);
                }
            }
            ...
            while (working) {
                ...
                int start, end, assinged_row;

                MPI_Recv(&assinged_row, 1, MPI_INT, MASTER, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
                int recieved_tag = status.MPI_TAG;

                if (recieved_tag == END_TAG) {
                    working = 0;
                }
                else {
                    start = assinged_row;
                    end = assinged_row + ROWS_PER_JOB;
 
                    if (start != 0) {
                        MPI_Recv(t + (start - 1) * col, 1, single_row, MASTER, ROW_TAG, MPI_COMM_WORLD, &status); 
                    }     
                    
                    MPI_Recv(t + start * col, ROWS_PER_JOB, single_row, MASTER, ROW_TAG, MPI_COMM_WORLD, &status); 

                    if (end != row) {
                        MPI_Recv(t + end * col, 1, single_row, MASTER, ROW_TAG, MPI_COMM_WORLD, &status); 
                    } 

                    single_iteration(r, t, power, row, col, Cap_1, Rx_1, Ry_1, Rz_1, step, start, end);

                    MPI_Send(&assinged_row, 1, MPI_INT, MASTER, RESULT_ROW_TAG, MPI_COMM_WORLD);
                    MPI_Send(r + start * col, ROWS_PER_JOB, single_row, MASTER, RESULT_TAG, MPI_COMM_WORLD);               
                }
            }
        }
...
int main(int argc, char **argv) {
    ...
    MPI_Bcast(power, grid_rows * grid_cols, MPI_FLOAT, MASTER, MPI_COMM_WORLD);
